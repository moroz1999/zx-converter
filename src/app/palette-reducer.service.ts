...
type SplitRgbColor = FixedSizeArray<3, number>;
...
export class PaletteReducerService {
  private readonly zxPalette: StringIndex<number> = {
    '0000': 0x000000,
    '0001': 0x0000CD,
    '0010': 0xCD0000,
    '0011': 0xCD00CD,
  ...
  private readonly pairColors: StringIndex<string> = {
    '0000': '1000',
    '0001': '1001',
    '0010': '1010',
    '0011': '1011',
  ...
  private processAttribute(attrX: number, attrY: number): void {
    ...
    for (let y = attrY * 8; y < attrY * 8 + 8; y++) {
      for (let x = attrX * 8; x < attrX * 8 + 8; x++) {
        let color = PaletteReducerService.imageColorAt(this.imageData, x, y);
        colors.push({
          'color': color,
          'x': x,
          'y': y,
    ...
    for (let i in colors) {
      let info = colors[i];
      if (PaletteReducerService.colorDiff(info['color'], this.splitPalette[maxColor]) < PaletteReducerService.colorDiff(info['color'], this.splitPalette[minColor])) {
        PaletteReducerService.imageSetPixel(this.resultImageData, info['x'], info['y'], this.splitPalette[maxColor]);
        this.setChrdPixel(true, info['x'], info['y'], maxColor);
      } else {
        PaletteReducerService.imageSetPixel(this.resultImageData, info['x'], info['y'], this.splitPalette[minColor]);
